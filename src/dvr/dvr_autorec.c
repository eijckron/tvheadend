...
autorec_cmp(dvr_autorec_entry_t *dae, epg_broadcast_t *e)
{
  ...
  if(dae->dae_content_type.code != 0) {
    ...
    ev_minutes = ev_time.tm_hour * 60 + ev_time.tm_min;

	  if(dae->dae_tod_after < dae->dae_tod_before) {
	  	if ((ev_minutes < dae->dae_tod_after) || (ev_minutes > dae->dae_tod_before))
	  		return 0;
	  } else {
	  	if ((ev_minutes < dae->dae_tod_after) && (ev_minutes > dae->dae_tod_before))
	  		return 0;
...
autorec_record_build(dvr_autorec_entry_t *dae)
{
  ...
  htsmsg_add_u32(e, "tod_after", dae->dae_tod_after);
  htsmsg_add_u32(e, "tod_before", dae->dae_tod_before);
		      ...
		      int maycreate)
{
  ...
  if((s = htsmsg_get_str(values, "approx_time")) != NULL) {
    ...
    if(strchr(s, ':') != NULL) {
      ...
      approx_time = (atoi(s) * 60) + atoi(s + 3);
    } else if(strlen(s) == 0) {
      approx_time = 0;
    } else {
      approx_time = atoi(s);
  ...
  if((s = htsmsg_get_str(values, "tod_after")) != NULL) {
    if(strchr(s, ':') != NULL) {
      // formatted time string - convert
      dae->dae_tod_after = (atoi(s) * 60) + atoi(s + 3);
    } else if(strlen(s) == 0) {
      dae->dae_tod_after = 0;
    } else {
      dae->dae_tod_after = atoi(s);
    }
  }
  ...
  if((s = htsmsg_get_str(values, "tod_before")) != NULL) {
    if(strchr(s, ':') != NULL) {
      // formatted time string - convert
      dae->dae_tod_before = (atoi(s) * 60) + atoi(s + 3);
    } else if(strlen(s) == 0) {
      dae->dae_tod_before = 0;
    } else {
      dae->dae_tod_before = atoi(s);
    }
  }
